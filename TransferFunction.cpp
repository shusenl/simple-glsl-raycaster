...
void TransferFunction::SavetoFile(const char *path)
{
    ...
    if(!outfile) {
        std::cerr << path << ": output file can't open!" << std::endl;
        return;
    }
    std::ostream_iterator<Key> outit(outfile, "\n");
    std::copy(Keylist.begin(), Keylist.end(), outit);
    outfile.close();
}
...
void TransferFunction::LoadfromFile(const char *path)
{
  Keylist.clear();
    ...
    if(!infile) {
        std::cerr << path << ": input file not found!" << std::endl;
        return;
    }
    std::istream_iterator<Key> init(infile);
    std::istream_iterator<Key> end;
    ...
    for(int i=0; i<TABLESIZE; i++)
    {
      if(j!=i)
      {
        ...
        if(j>i)
        {
          ...
          {
            float alpha = float(colortable[int(ii)][3])/255.0f;
            preIntegrateTable[j][i][0] += alpha*(float(colortable[int(ii)][0])/255.0f);
            preIntegrateTable[j][i][1] += alpha*(float(colortable[int(ii)][1])/255.0f);
            preIntegrateTable[j][i][2] += alpha*(float(colortable[int(ii)][2])/255.0f);
            alphaAccum += alpha;
         }
        }
        else
        ...
        */
        ...
        if(j>i)
        {
          for(float fj = float(j); fj>float(i); fj-=delta)
          {
            ...
            preIntegrateTable[j][i][0] += (1-preIntegrateTable[j][i][3])*(float(colortable[int(fj)][0])/255.0f)*alpha;
            preIntegrateTable[j][i][1] += (1-preIntegrateTable[j][i][3])*(float(colortable[int(fj)][1])/255.0f)*alpha;
            preIntegrateTable[j][i][2] += (1-preIntegrateTable[j][i][3])*(float(colortable[int(fj)][2])/255.0f)*alpha;

            preIntegrateTable[j][i][3] += (1-preIntegrateTable[j][i][3])*alpha;
          }
        }
        else
        {
        ...
        if(preIntegrateTable[j][i][3]!=0.0)
        {
          //convert back to C for c
          preIntegrateTable[j][i][0] /= preIntegrateTable[j][i][3];
      ...
      {
        preIntegrateTable[j][i][0]=float(colortable[i][0])/255.0f;
        preIntegrateTable[j][i][1]=float(colortable[i][1])/255.0f;
        preIntegrateTable[j][i][2]=float(colortable[i][2])/255.0f;
        preIntegrateTable[j][i][3]=float(colortable[i][3])/255.0f;
      }
  }
